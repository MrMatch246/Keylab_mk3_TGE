...
def make_button_segment(button_index, text=None, button_type=BUTTON_TYPES['icon'][0], color=Color.WHITE, icon=Icon.NONE):
    pass
    return DISPLAY_HEADER + (button_index,) + (0, button_type,)+ (0, 3, icon)\
     + (0,4)+  color + (0, 5) + (text if text else (0,)) + (0, SYSEX_END)


def make_two_line_screen(line_1, line_2, color=Color.WHITE):
    pass
    return DISPLAY_HEADER + (ScreenId.TWO_LINE,) + \
           make_line_bytes(line_1, color) + \
           make_line_bytes(line_2, Color.WHITE, address=3) + \
           (SYSEX_END,)
...
def make_popup_screen(line_1, line_2, line_3=None, colors=(Color.WHITE, Color.WHITE, Color.WHITE), icon=Icon.NONE):
    ...
    (ScreenId.THREE_LINE_POPUP if line_3 else ScreenId.ICONIFIED_TWO_LINE_POPUP if icon else ScreenId.TWO_LINE_POPUP,)\
     + make_line_bytes(line_1, colors[0])\
     + make_line_bytes(line_2, colors[1], address=3)\
