...
def create_mixer_popup_screen(state):
    if state.continuous_control_modes.selected_mode == 'mixer':
        ...
        'HOLD TO OFFSET'), popup_colors=(Color.WHITE, Color.BLUE, Color.GREY))
    return None
...
def create_mixer_button_content(state):
    ...
    return (ButtonContent(text=simple_track_name(track),
                            on_color=Color.YELLOW,
                            is_on=track != song().master_track and (not track.mute),
                          button_type=BUTTON_TYPES['toggle']),
            ButtonContent(icon=Icon.SOLO, on_color=Color.BLUE,is_on=track != song().master_track and (track.solo)),
            ButtonContent(icon=Icon.MIDI_ARM if track.has_midi_input else Icon.AUDIO_ARM,on_color=Color.RED,is_on=is_track_armed(track)),
            ButtonContent(icon=Icon.STOP, on_color=Color.RED,is_on=True)\
             if state.scene_launch.launch_button.is_held else\
              ButtonContent(icon=Icon.PLAY, on_color=Color.GREEN,is_on=song().view.selected_scene.is_triggered)
              )
...
def create_modifier_button_content(state, modifier_name):

    return ButtonContent(text=modifier_name.upper(),
                         is_on=False,  #TODO add variable for modifier state to state
                         button_type=BUTTON_TYPES['tab']
                         )
